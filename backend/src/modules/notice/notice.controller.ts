...
export const createNotice = asyncHandler(async (req: Request, res: Response) => {
  const notice = await noticeService.createNotice(req.body, req.file);

  return res.status(201).json(new ApiResponse(201, notice, 'Notice created successfully'));
});

// GET ALL - ADMIN
export const getAllNotices = asyncHandler(async (_req: Request, res: Response) => {
...
export const getPublishedNotices = asyncHandler(async (_req: Request, res: Response) => {
  const notices = await noticeService.getPublishedNotices();

  return res
    .status(200)
    .json(new ApiResponse(200, notices, 'Published notices fetched successfully'));
});
...
export const updateNotice = asyncHandler(async (req: Request, res: Response) => {
  ...
  const notice = await noticeService.updateNotice(noticeId, req.body, req.file);
